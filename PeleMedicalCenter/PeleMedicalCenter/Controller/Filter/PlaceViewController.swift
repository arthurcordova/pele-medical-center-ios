...
class PlaceViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func selectedPlace(_ sender: Any) {
        if (whatIs == "city") {
            ...
            self.defaultsFilter.setValue(city.uuid, forKey: "city_id")
            self.defaultsFilter.setValue(city.name, forKey: "city_name")
            self.defaultsFilter.synchronize()
            
            self.buttonFromView?.setTitle(self.defaultsFilter.string(forKey: "city_name"), for: .normal)
